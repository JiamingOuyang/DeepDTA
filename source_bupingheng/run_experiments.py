...
def general_nfold_cv(XD, XT, Y, label_row_inds, label_col_inds, prfmeasure, runmethod, FLAGS, labeled_sets, val_sets):  ## BURAYA DA FLAGS LAZIM????
    ...
    #for foldind in range(len(val_sets)):
        ...
        XD_train = XD
        XT_train = XT
        trrows = label_row_inds[labeledinds]#val
        ...
        train_drugs3 = pd.read_table('train_drugs3.txt', header=None)
        train_prots3 = pd.read_table('train_prots3.txt', header=None)
        test_drugs3 = pd.read_table('test_drugs3.txt', header=None)
        test_prots3 = pd.read_table('test_prots3.txt', header=None)
        train_Y3 = pd.read_table('train_Y3.txt', header=None)
        val_Y3 = pd.read_table('test_Y3.txt', header=None)
        ...
        for param1ind in range(len(paramset1)):  # hidden neurons
            ...
            for param2ind in range(len(paramset2)):  # learning rate
                ...
                for param3ind in range(len(paramset3)):
                    ...
                    # train_drugs2 = pd.DataFrame(train_drugs)
                    # train_drugs2.to_csv('train_drugs.txt', sep='\t', header=None, index=False)
                    # train_prots2 = pd.DataFrame(train_prots)
                    # train_prots2.to_csv('train_prots.txt', sep='\t', header=None, index=False)
                    # train_Y2 = pd.DataFrame(train_Y)
                    # train_Y2.to_csv('train_Y.txt', sep='\t', header=None, index=False)
                    # val_drugs2 = pd.DataFrame(val_drugs)
                    # val_drugs2.to_csv('val_drugs.txt', sep='\t', header=None, index=False)
                    # val_prots2 = pd.DataFrame(val_prots)
                    # val_prots2.to_csv('val_prots.txt', sep='\t', header=None, index=False)
                    # val_Y2 = pd.DataFrame(val_Y)
                    # val_Y2.to_csv('val_Y.txt', sep='\t', header=None, index=False)

                    # a = [np.array(train_drugs), np.array(train_prots)]
                    # b = np.array(train_Y)
                    # c = [np.array(val_drugs), np.array(val_prots)]
                    # d = np.array(val_Y)
                    a = np.array(train_drugs3)
                    b = np.array(train_prots3)
                    c = np.array(test_drugs3)
                    d = np.array(test_prots3)
                    train_Y = np.squeeze(np.array(train_Y3))
                    train_Y_one_hot = (np.arange(train_Y.max() + 1) == train_Y[:, None]).astype(dtype='int')
                    val_Y = np.squeeze(np.array(val_Y3))
                    val_Y_one_hot = (np.arange(val_Y.max() + 1) == val_Y[:, None]).astype(dtype='int')

                    gridres = gridmodel.fit(([np.array(train_drugs3), np.array(train_prots3)]), train_Y_one_hot,
                                            batch_size=batchsz, epochs=epoch,
                                            validation_data=(
                                                ([np.array(test_drugs3), np.array(test_prots3)]), val_Y_one_hot),
                    ...
                    #
                    #
                    # gridres = gridmodel.fit(([np.array(train_drugs), np.array(train_prots)]), train_Y_one_hot,
                    #                         batch_size=batchsz, epochs=epoch,
                    #                         validation_data=(
                    #                         ([np.array(val_drugs), np.array(val_prots)]), val_Y_one_hot),
                    #                         shuffle=False, callbacks=[es])

                    all_drugs3 = pd.read_table('all_drugs3.txt', header=None)
                    all_prots3 = pd.read_table('all_prots3.txt', header=None)
                    permute_layer_model = Model(input=gridmodel.input,output=gridmodel.get_layer('dense_3').output)
                    permute_layer_output = permute_layer_model.predict([np.array(all_drugs3),np.array(all_prots3) ])
                    ...
                    predicted_labels = gridmodel.predict([np.array(test_drugs3), np.array(test_prots3)])
