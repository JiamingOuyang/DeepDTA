...
class DataSet(object):
    ...
    def read_sets(self, FLAGS):  ### fpath should be the dataset folder /kiba_old/ or /davis/
        ...
        # test_fold2 = pd.DataFrame(test_fold)
        # test_fold2.to_csv('test_fold.txt', sep='\t', header=None, index=False)
        # train_folds2 = pd.DataFrame(train_folds)
        # train_folds2.to_csv('train_folds.txt', sep='\t', header=None, index=False)
    ...
    def parse_data(self, FLAGS, with_label=True):
        ...
        Y2 = pd.read_table('Y2_2.txt', header=None)
        Y2 = np.asarray(Y2)
        XDd = pd.read_table('XD2.txt', header=None, names=['name', 'smile'])
        XTt = pd.read_table('XT2.txt', header=None, names=['name', 'seq'])
        for index, row in XDd.iterrows():
            a = row['smile']
            b = str(a)
            XD2.append(label_smiles(row['smile'], self.SMILEN, self.charsmiset))
        for index, row in XTt.iterrows():
            a = row['seq'][0:100]
            b = row['seq']
            XT2.append(label_sequence(row['seq'], self.SEQLEN, self.charseqset))
        # XD2 = pd.DataFrame(XD2)
        # XD2.to_csv('XD2.txt', sep='\t', header=None, index=False)
        # XT2 = pd.DataFrame(XT2)
        # XT2.to_csv('XT2.txt', sep='\t', header=None, index=False)
        # Y3 = pd.DataFrame(Y)
        # Y3.to_csv('Y2.txt', sep='\t', header=None, index=False)

        # Y2 = pd.read_table('Y2.txt', header=None)
        # Y2 = np.asarray(Y2)
        # XDd = pd.read_table('durg_smile_1941.txt', header=None, names=['name', 'smile'])
        # XTt = pd.read_table('bio_seq_148.txt', header=None, names=['name', 'seq'])
        # for index, row in XDd.iterrows():
        #     a = row['smile']
        #     b = str(a)
        #     XD2.append(label_smiles(row['smile'], self.SMILEN, self.charsmiset))
        ...
        #     XT2.append(label_sequence(row['seq'], self.SEQLEN, self.charseqset))
        # XD2 = pd.DataFrame(XD2)
        # XD2.to_csv('durg_smile_feature100.txt', sep='\t', header=None, index=False)
        # XT2 = pd.DataFrame(XT2)
        # XT2.to_csv('bio_seq_feature1000.txt', sep='\t', header=None, index=False)
        # # Y3 = pd.DataFrame(Y)
        # # Y3.to_csv('Y2.txt', sep='\t', header=None, index=False)
        return XD2, XT2, Y2
